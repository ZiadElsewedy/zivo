...
/**
 * The `thoughtSignature` Google documents for a function call injected into
 * the history rather than generated by Gemini in that conversation — here, a
 * tool call Claude made before a mid-turn fallback to Gemini
 * (https://ai.google.dev/gemini-api/docs/thought-signature).
 * @const {string}
 */
  ...
  if (part && part.type === "raw") {
    ...
    if (raw && raw.type === "tool_use") {
      return {
        ...
        // Gemini 3 rejects a model turn's functionCall that carries no
        // `thoughtSignature` with a 400 ("Function call is missing a
        // thought_signature") — and a call Claude made has none. This is
        // Google's documented value for a function call Gemini didn't
        // generate itself (history from another model); without it every
        // mid-turn Claude→Gemini fallback after a tool call failed.
        thoughtSignature: FOREIGN_CALL_THOUGHT_SIGNATURE,
      };
    }
    return null;
  }
